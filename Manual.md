...
The `lazy-pkgs` repository contains all the
...
in a directory matching `$pkgname`, Example: `lazy-pkgs/srcpkgs/foo/template`.
...
- `XBPS_BUILD_ENVIRONMENT` Enables continuous-integration-specific operations. Set to `lazy-pkgs-ci` if in continuous integration.
...
`lazy-pkgs/common/build-style` directory. Please note that required packages
...
To get started, [fork](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-forks/fork-a-repo) the lazylinuxos `lazy-pkgs` git repository on GitHub and clone it:

    $ git clone git@github.com:<user>/lazy-pkgs.git
...
to pull in new changes:
    ...
    $ git remote add upstream https://github.com/lazylinuxos/lazy-pkgs.git
